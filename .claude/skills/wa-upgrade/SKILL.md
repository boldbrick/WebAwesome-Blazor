...
1. Run `tools\upgrade\Sync-WaDocs.ps1 -Version <target>` — it downloads the public GitHub repo at tag `v<target>` (cached codeload zip), replaces `inputs\WebAwesome` with its `packages/webawesome/docs/docs` tree, and fills doc gaps (components in the target CEM but absent from the GitHub tree, i.e. Pro components) from the reference docs bundled in the release zip (`dist/skills/webawesome/references/components/`, present since 3.3.0), falling back to carrying the existing doc forward with a re-stamped source header.
2. For every component the script reports as **NEEDS CAPTURE** (no bundled reference and no existing doc to carry forward), fetch its doc page from the reported `https://webawesome.com/docs/components/<name>` URL, convert it to markdown, and save it as `inputs\WebAwesome\components\<name>.md` with the source URL noted in a comment at the top (format: see `components\combobox.md`).
...
**Pro-source rule (hard):** everything extracted from the Pro release zips stays under `temp\` (ignored by Plastic and git), with exactly two sanctioned exceptions ever checked in: the API surface JSON (names, types, defaults, doc descriptions — no implementation code) and the bundled reference **documentation** markdown (`dist/skills/webawesome/references/components/*.md`, used by `Sync-WaDocs.ps1` to fill `inputs\WebAwesome` gaps — publicly documented content, owner-approved). Plan documents and migration guides may describe APIs but must never embed upstream JS/TS/CSS source.
...
4. Demo pages and showcases — **new components ship with curated demo coverage in the same run**; a TODO-marked skeleton is not a deliverable:
   1. Run `tools\demo\New-WaDemoPages.ps1 -PruneRemoved` — new components get skeleton pages, removed components' pages are deleted.
   2. Curate each new component's page: translate the examples from `inputs\WebAwesome\components\<name>.md` to the Blazor wrappers, following the curated-page pattern (e.g. `ComboboxPage.razor`) — `ComponentBadges` in the heading, a one-sentence intro, several `ExampleSection` blocks whose raw-string `Code` constants mirror the live markup, `ApiTable` at the end, TODO marker removed. Use only parameters that actually exist on the wrapper; record genuine wrapper API gaps in the plan document rather than working around them. Delegate curation in parallel groups (≤5 pages per agent) when agent dispatch is available.
   3. Review the showcase pages (`src\WebAwesome.Blazor.Demo\Pages\Showcases\`, when present): components **removed** by the upgrade must be removed from showcases (they would break the build); components **added** must be worked into the showcase where they naturally fit in the same run (form controls → the form showcase; data/visualization → the dashboard; overlays/notifications → the overlays showcase; content rendering → the content showcase) — extend each showcase's existing narrative and idioms, don't restructure it. Only a component with no natural home in any existing showcase may be deferred, noted in the plan document with the reason.
   4. The demo project must build; the browser sweep (step 7) must cover the curated pages and extended showcases green.
