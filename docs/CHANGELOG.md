...
- Demo navigation: the three new components were added to `ComponentCategoryMap` — `wa-otp-input` and `wa-data-grid` under Forms, `wa-pagination` under Navigation, matching the webawesome.com components index. Without this they fell into an unmapped "Other" group, which the Playwright taxonomy check caught.
